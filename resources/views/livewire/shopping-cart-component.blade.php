...
new class extends Component {
    ...
    public string $searchTerm = ''; // Termo de busca
    ...
    public function products()
    {
        ...
        // Aplica filtro de busca pelo nome do produto
    ...
    {
        $this->searchTerm = ''; // Limpa o termo de busca ao mudar a categoria
        $this->selectedCategory = $category;
    }

    public function searchMulti($search)
    {
        $this->searchTerm = $search; // Atualiza o termo de busca dinamicamente
    }
    ...
    <x-mary-header size="text-inherit" progress-indicator>
        ...
        <x-slot:title>
            <x-mary-choices
                ...
                no-result-text="Ops! Nada encontrado..."
                searchable
        ...
        <x-slot:actions class="mt-7">
            <x-mary-dropdown label="Times Europeus" class="lg:btn btn-sm">
                <x-mary-menu-item @click.stop value="liga_alema" title="Liga Alemã" wire:click="setFilter('liga_alema')" spinner="setFilter('liga_alema')" />
            ...
            {{-- Seleções Nacionais --}}
            <x-mary-dropdown label="Seleções" class="lg:btn btn-sm">
            ...
            {{-- Times Brasileiros --}}
            <x-mary-dropdown label="Times Brasileiros" class="lg:btn btn-sm">
        ...
        <div x-data class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            @forelse($products as $product)
                <x-mary-card wire:key="product-{{ $product->id }}" title="{{ $product->name }}" class="dark:bg-gray-800 bg-gray-50 text-sm relative shadow-md">
                    <span class="inline-flex items-start gap-1 top-2 left-2 absolute rounded-md bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-700 ring-1 ring-yellow-400 dark:ring-yellow-600 ring-inset">
                    ...
                    <x-slot:figure class="relative">
                        <div x-data="{ loaded: false }" class="w-full h-auto rounded-md relative">
                            ...
                            <div x-show="!loaded" class="absolute inset-0 flex items-center justify-center bg-gray-200 animate-pulse rounded-md">
                            ...
                            <img 
                                src="{{ asset('storage/' . ($product->image[0] ?? 'default.jpg')) }}" 
                                class="h-72 w-full" 
                                alt="{{ $product->name }}" 
                                x-data="{ loaded: false }"
                        ...
                        <span class="inline-flex bottom-2 right-2 absolute items-start rounded-md text-start bg-green-100 px-2 py-1 text-xs gap-1 font-medium dark:text-green-500 text-green-600 ring-1 ring-green-600/20 ring-inset">
